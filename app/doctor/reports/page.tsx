...
export default function DoctorReportsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Fetch all scans for patients belonging to this doctor
      ...
      // Filter to only show scans for this doctor's patients
      const filtered = (data || []).filter((scan: any) =>
        scan.patients?.assigned_doctor_id === user.id || scan.uploader_id === user.id
      );

      const rows: ReportRow[] = filtered.map((scan: any) => {
        ...
        return {
          ...
          patientId: patient?.patient_id ?? scan.id,
          ...
          evaluationNotes: scan.evaluation_notes ?? undefined,
  ...
  const filtered = reports.filter(r => {
    ...
    const matchSearch = r.patientName.toLowerCase().includes(s) || r.patientId.toLowerCase().includes(s) || r.fileName.toLowerCase().includes(s);
  ...
  const handleDownloadPDF = async (report: ReportRow) => {
    ...
    try {
      ...
      const evaluationHtml = report.evaluation ? `
        <div style="background:${diagColor}08;border:1px solid ${diagColor}22;border-radius:8px;padding:16px;margin-bottom:20px;">
          <p style="margin:0 0 8px;font-size:10px;font-weight:700;color:#6B98BA;letter-spacing:1px;text-transform:uppercase;">Clinician Evaluation</p>
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
            <span style="font-size:11px;color:#64748B;">Ground Truth:</span>
            <span style="font-size:13px;font-weight:700;color:${DIAG_COLOR[report.evaluation] ?? "#0284C7"};">${report.evaluation}</span>
            ${report.evaluation === report.primaryDiagnosis
              ? '<span style="background:#ECFDF5;color:#059669;font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;">AI CONFIRMED</span>'
              : '<span style="background:#FEF3C7;color:#B45309;font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;">AI CORRECTED</span>'
            }
          </div>
          ${report.evaluationNotes ? `<p style="margin:0;font-size:11px;color:#355878;font-style:italic;">"${report.evaluationNotes}"</p>` : ""}
        </div>` : "";
        ...
        <div style="display:flex;gap:16px;margin-bottom:20px;">
          <div style="flex:1;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:14px 16px;">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:11px;">
              ...
              <tr><td style="padding:4px 0;color:#64748B;">DOB</td><td style="padding:4px 0;font-weight:600;">${report.dateOfBirth !== "N/A" ? new Date(report.dateOfBirth).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "N/A"}</td></tr>
            </table>
          </div>
          <div style="flex:1;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:14px 16px;">
      ...
      await html2pdf().set({
        ...
        filename: `NeuroScan_Report_${report.patientName.replace(/ /g, "_")}_${report.scanId}.pdf`,
  ...
  return (
    <div className="flex-1 p-8 overflow-auto">
      {/* Header */}
      <div className="mb-7">
        <h1 className="text-[24px] font-bold text-[#0A2540] tracking-[-0.5px]">All Reports</h1>
        <p className="text-[13px] text-[#6B98BA] mt-[3px]">
          {loading ? "Loading…" : `${reports.length} scans · download PDF reports with clinician evaluation`}
        </p>
      </div>
        ...
        <div className="flex-1 min-w-0 bg-white/55 backdrop-blur-[20px] border border-white/75 rounded-[16px] shadow-[0_4px_24px_rgba(14,165,233,0.08)] overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {["Patient", "Scan File", "Date", "Diagnosis", "Confidence", "Status", "Actions"].map((h) => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-[18px] py-[28px] text-center text-[13px] text-[#6B98BA]">
                      ...
                      <tr key={report.scanId} className="border-b border-[rgba(14,165,233,0.06)] last:border-b-0 hover:bg-white/40 transition-colors duration-150 group">
                        ...
                        <td className="px-[18px] py-[12px]">
                          <div className="flex items-center gap-[10px]">
                            <div className="w-[32px] h-[32px] rounded-full flex items-center justify-center text-[12px] font-bold flex-shrink-0 text-white" style={{ background: getAvatarGradient(report.patientId) }}>
                              {getInitials(report.patientName)}
                            </div>
                            <div>
                              <p className="text-[13.5px] font-semibold text-[#0A2540]">{report.patientName}</p>
                              <p className="text-[11px] text-[#6B98BA]">{report.patientId}</p>
                            </div>
                          </div>
                        </td>

                        <td className="px-[18px] py-[12px]">
                          <p className="text-[13.5px] text-[#0A2540] font-semibold truncate max-w-[150px]">{report.fileName}</p>
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            {report.evaluation && (
                              <div className="flex items-center gap-1">
                                <svg className="w-3 h-3 stroke-emerald-500 fill-none" viewBox="0 0 24 24" strokeWidth={2.5} strokeLinecap="round">
                                  <path d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span className="text-[10.5px] font-semibold" style={{ color: DIAG_COLOR[report.evaluation] ?? "#059669" }}>
                                  {report.evaluation === report.primaryDiagnosis ? "Confirmed" : `→ ${report.evaluation}`}
                                </span>
                              </div>
                            )}
                          </div>
                        ...
                        >
                            ...
                            <a
                              href={`/doctor/analysis?scanId=${report.scanId}`}
